...
export default function SettingsPage() {
    ...
    const fetchData = async () => {
      try {
        ...
        setSettings({
          ...
          schema_type: siteData.schema_type || "Restaurant",
        })
  ...
  const handleSave = async (data: any = {}) => {
    ...
    try {
      ...
      formData.append("schema_type", settings.schema_type)
    ...
    <div className="space-y-6">
      ...
      <Tabs defaultValue="general" className="space-y-6" dir="rtl">
        <TabsList className="w-full justify-start md:w-auto">
          ...
          <TabsTrigger value="seo">تنظیمات SEO</TabsTrigger>
        ...
        <TabsContent value="seo">
          <Card>
            <CardHeader className="text-right">
              ...
              <CardDescription>
                تنظیمات مربوط به موتورهای جستجو را برای بهبود رتبه سایت خود مدیریت کنید
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-6 text-right">
              <div className="space-y-2">
                <Label htmlFor="meta_title" className="text-right block">عنوان سئو (Meta Title)</Label>
                <Input
                  id="meta_title"
                  value={settings.meta_title}
                  onChange={(e) => setSettings({ ...settings, meta_title: e.target.value })}
                  placeholder="عنوان نمایشی در گوگل"
                  className="text-right"
                />
                <p className="text-xs text-muted-foreground">بهترین طول بین ۵۰ تا ۶۰ کاراکتر است.</p>
              </div>

              <div className="space-y-2">
                <Label htmlFor="meta_description" className="text-right block">توضیحات سئو (Meta Description)</Label>
                <Textarea
                  id="meta_description"
                  value={settings.meta_description}
                  onChange={(e) => setSettings({ ...settings, meta_description: e.target.value })}
                  placeholder="توضیحات کوتاهی که در نتایج جستجو نمایش داده می‌شود"
                  rows={3}
                  className="text-right"
                />
                <p className="text-xs text-muted-foreground">بهترین طول بین ۱۵۰ تا ۱۶۰ کاراکتر است.</p>
              </div>

              <div className="space-y-2">
                <Label htmlFor="schema_type" className="text-right block">نوع کسب‌وکار (Schema Type)</Label>
                <select
                  id="schema_type"
                  ...
                  dir="ltr"
                >
                  <option value="Restaurant">رستوران (Restaurant)</option>
                  <option value="CafeOrCoffeeShop">کافه (CafeOrCoffeeShop)</option>
                  <option value="FastFoodRestaurant">فست فود (FastFoodRestaurant)</option>
                  <option value="Bakery">نانوایی/قنادی (Bakery)</option>
                  <option value="BarOrPub">بار (BarOrPub)</option>
                </select>
                <p className="text-xs text-muted-foreground">این مورد به موتورهای جستجو کمک می‌کند تا ماهیت کسب‌وکار شما را بهتر درک کنند.</p>
              </div>
