import LegalPage from "@/components/legal-layout"
import { Metadata } from "next"

export const metadata: Metadata = {
  title: "شرایط استفاده | وفینو",
  description: "قوانین و مقررات استفاده از خدمات پلتفرم وفینو",
}

export default function TermsPage() {
  return (
    <LegalPage 
      title="شرایط استفاده"
      content={
        <>
          <p>با استفاده از خدمات وفینو، شما موافقت خود را با شرایط زیر اعلام می‌کنید:</p>
          <h2 className="text-xl font-bold mt-8 mb-4">قوانین کلی</h2>
          <p>استفاده از وفینو باید مطابق با قوانین جمهوری اسلامی ایران باشد.</p>
          <h2 className="text-xl font-bold mt-8 mb-4">مالکیت معنوی</h2>
          <p>تمامی حقوق مادی و معنوی پلتفرم وفینو متعلق به این شرکت است.</p>
          <h2 className="text-xl font-bold mt-8 mb-4">تعهدات کاربر</h2>
          <p>کاربر متعهد می‌شود که اطلاعات صحیح را در سامانه وارد کرده و از سوءاستفاده از امکانات پلتفرم خودداری کند.</p>
        </>
      }
    />
  )
}
